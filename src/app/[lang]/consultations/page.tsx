...
export async function generateMetadata({params}: {params: Promise<{lang: string}>}) {
  ...
  const page = (locale as {consultationsPage?: {metaTitle?: string; metaDescription?: string}}).consultationsPage;
  return {
    title: page?.metaTitle ?? 'Consultations | Astrolog Umran',
    description: page?.metaDescription ?? undefined,
  };
}

export default async function ConsultationsPage({params}: {params: Promise<{lang: string}>}) {
  const {lang} = await params;
  ...
  return (
    <main className="max-w-[900px] mx-auto px-4 sm:px-6 py-10 sm:py-16">
      <h1 className="font-cinzel text-3xl md:text-4xl theme-text tracking-wide mb-4">
        {page.title || t('nav.consultations')}
      </h1>
