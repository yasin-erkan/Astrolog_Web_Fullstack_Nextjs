...
export async function generateMetadata({params}: {params: Promise<{lang: string}>}) {
  ...
  const page = (locale as {dailyRoutinesPage?: {metaTitle?: string; metaDescription?: string}}).dailyRoutinesPage;
  return {
    title: page?.metaTitle ?? 'Daily Rituals | Astrolog Umran',
  ...
  return (
    <main className="max-w-[900px] mx-auto px-4 sm:px-6 py-10 sm:py-16">
      <h1 className="font-cinzel text-3xl md:text-4xl theme-text tracking-wide mb-6">
        {page.title || t('nav.dailyRoutines')}
      </h1>
